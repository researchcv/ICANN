...
@dataclass
class Evidence:
    """A single piece of evidence supporting a spatial relation."""
    source: str              # "gaussian_geometry" | "yolo_multiview" | "llm_reasoning"
...
class SpatialRelation:
    """Spatial relation with optional multi-source evidence chain."""
    subject_id: int
    predicate: str
    object_id: int
    distance: float
    ...
    def evidence_summary(self) -> str:
        """One-line summary of supporting evidence for LLM prompts."""
        if not self.evidence_chain:
            return "no evidence"
        parts = [f"{e.source}({e.score:.2f})" for e in self.evidence_chain]
        return ", ".join(parts)
